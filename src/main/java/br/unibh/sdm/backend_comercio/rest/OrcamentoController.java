...
public class OrcamentoController {
    ...
    public Orcamento createOrcamento(@RequestBody @NotNull Orcamento orcamento) throws Exception {
         return orcamentoService.criandoOrcamento(orcamento);
    }
    
    @PutMapping(value = "{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public Orcamento updateOrcamento(@PathVariable String id, 
    		@RequestBody @NotNull Orcamento orcamento) throws Exception {if (!id.equals(orcamento.getId())) {
                throw new Exception("orcamento " + id + " nao está correto");
            }
